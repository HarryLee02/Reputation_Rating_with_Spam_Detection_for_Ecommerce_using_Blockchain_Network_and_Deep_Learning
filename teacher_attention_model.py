...
# -----------------------------------------------------------
class TeacherAttention(T.nn.Module):
    """
    Teacher's suggested attention mechanism: softmax((q @ a @ q^T )/sqrt(dim)) @ v
    where 'a' is a learnable attention matrix
    """
    def __init__(self, embed_dim, num_heads):
        ...
        # Learnable attention matrix 'a' for each head
        # Shape: [num_heads, head_dim, head_dim]
        self.attention_matrix = T.nn.Parameter(T.randn(num_heads, self.head_dim, self.head_dim) * 0.1)
    ...
    def forward(self, x):
        ...
        # attention formula: softmax((q @ a @ q^T )/sqrt(dim)) @ v
        # q shape: [batch_size, num_heads, seq_len, head_dim]
        # a shape: [num_heads, head_dim, head_dim]
        # q @ a shape: [batch_size, num_heads, seq_len, head_dim]
        # (q @ a) @ q^T shape: [batch_size, num_heads, seq_len, seq_len]
        
        # Compute q @ a for each head
        q_a = T.matmul(q, self.attention_matrix.unsqueeze(0).expand(batch_size, -1, -1, -1))
        
        # Compute (q @ a) @ q^T
        scores = T.matmul(q_a, q.transpose(-2, -1)) / T.sqrt(T.tensor(self.head_dim, dtype=T.float32))
...
# -----------------------------------------------------------
class TeacherAttentionModel(T.nn.Module):
    def __init__(self, vocab_size, embed_dim=128, num_layers=4, num_heads=4):
        super(TeacherAttentionModel, self).__init__()
        # Embedding layer
        self.embed = MyEmbedding(vocab_size, embed_dim)
        
        # Teacher attention layers
        self.attention_layers = T.nn.ModuleList([
            TeacherAttention(embed_dim, num_heads) for _ in range(num_layers)
        ])
...
def plot_confusion_matrix(cm, save_path):
    ...
    plt.title('Confusion Matrix - Teacher Attention Model')
...
def main():
    print("\nBegin PyTorch Vietnamese Teacher Attention Model demo")
    gc.enable()
    ...
    unified_vocab.to_json('/kaggle/working/vocab_with_freq_updated_unified_teacher.json', orient='index', force_ascii=False)
    print("Unified vocab saved to /kaggle/working/vocab_with_freq_updated_unified_teacher.json")
    ...
    # Create teacher attention model
    print("\nCreating Teacher Attention Model")
    embed_dim = 128
    num_layers = 4
    num_heads = 4
    max_epochs = 40

    model = TeacherAttentionModel(vocab_size, embed_dim, num_layers, num_heads).to(device)
    print(f"Teacher Attention Model created with {num_layers} layers, {num_heads} heads per layer")
    print(f"Using attention formula: softmax((q @ a @ q^T )/sqrt(dim)) @ v")
    ...
    metrics_df.to_csv('/kaggle/working/training_metrics_teacher_attention.csv', index=False)
    ...
    plt.title('Training Loss Over Time - Teacher Attention', fontsize=12, pad=15)
    ...
    plt.title('Model Accuracy Over Time - Teacher Attention', fontsize=12, pad=15)
    plt.xlabel('Epoch', fontsize=10)
    ...
    plt.tight_layout()
    plt.savefig('/kaggle/working/training_curves_teacher_attention.png', dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
    ...
    plot_confusion_matrix(cm, '/kaggle/working/confusion_matrix_teacher_attention.png')
    print("Confusion matrix saved to /kaggle/working/confusion_matrix_teacher_attention.png")

    # Save model
    print("\nSaving trained model state")
    model_save_path = "/kaggle/working/teacher_attention_vietnamese_model.pt"
    save_model(model, vocab_size, model_save_path)

    print("\nEnd PyTorch Vietnamese Teacher Attention Model demo")
